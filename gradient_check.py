...
from kernels import quat_to_matrix, quat_grad_from_rot_grad, sh_eval, sh_grad
...
class TestGradients(unittest.TestCase):
    ...
    def test_grad_projected_gaussian_2d(self):
        ...
        cov_2d, _ = projected_gaussian_2d(pos, scale, quat_to_matrix(q), view_matrix)
        precision = np.linalg.inv(cov_2d)

        # upstream gradients; precision is symmetric so symmetrise dL_d_precision
        M = rng.standard_normal((2, 2))
        dL_d_precision = (M + M.T) / 2
        dL_d_mu = rng.standard_normal(2)

        def proj_loss(s=scale, q_=q, p=pos):
            cov, mu = projected_gaussian_2d(p, s, quat_to_matrix(q_), view_matrix)
        ...
        print("grad_projected_gaussian_2d")
    ...
    def test_grad_colored_rendering(self):
        ...
        cov_2d, mu_2d = projected_gaussian_2d(pos, scale, quat_to_matrix(q), view_matrix)
